 ...
 *      only support small http request/reply.
 */
...
static int handle_read_event(int sockfd)
{
	char buf[BUF_LEN];
	int ret;
	char *p;

	ret = ngflow_read(sockfd, buf, BUF_LEN);
	if (ret <= 0) {
		return ret;
	}

	if ((p = strstr(buf, "\r\n\r\n")) == NULL) {
		printf("Socket %d: Failed to parse HTTP req header, read bytes: %d\n", 
				sockfd, ret);
		return ret;
	}

	ngflow_write(sockfd, http_resp, strlen(http_resp));
	...
	do {
		...
		for (i = 0; i < ret; i++) {
			...
			if (events[i].data.fd == lis_fd) {
				...
				/* Read stream and send the related response. */
				handle_read_event(events[i].data.fd);

				ev.events = EPOLLIN | EPOLLOUT;
				ev.data.fd = events[i].data.fd;
				ngflow_epoll_ctl(epoll_fd, EPOLL_CTL_MOD, events[i].data.fd, &ev);
